...
class EV2Gym(gym.Env):
    ...
    def __init__(self,
        ...
        self.skip_dynamic_price = skip_dynamic_price
        ...
        self.wireless_config = self.config.get('wireless_charging', {})
        self.wireless_enabled = self.wireless_config.get('enabled', False)
        self.price_forecast_config = self.config.get('price_forecast', {})
        self.price_forecast_enabled = self.price_forecast_config.get('enabled', False)
        ...
        if load_from_replay_path is not None:
            ...
            if self.wireless_enabled:
                self.cs = self.wireless_config.get('number_of_zones', cs)
            else:
                self.cs = cs  # Number of charging stations
            # Threshold for the user satisfaction score

            if self.wireless_enabled:
                self.number_of_ports_per_cs = self.wireless_config.get(
                    'zone_n_ports',
                    self.config['number_of_ports_per_cs'],
                )
            else:
    ...
    def reset(self, seed=None, options=None, **kwargs):
        ...
        if not self.skip_dynamic_price:
            self._apply_dynamic_discharge_prices()
        if self.price_forecast_enabled:
            try:
                if not hasattr(self, "_price_forecast_cached") or not self._price_forecast_cached:
                    from ev2gym.utilities.price_forecast import generate_price_forecast
                    horizon = int(self.price_forecast_config.get('horizon', 20))
                    lookback = int(self.price_forecast_config.get('lookback', 48))
                    model_name = self.price_forecast_config.get('model', 'linear')
                    hidden_size = int(self.price_forecast_config.get('hidden_size', 32))
                    epochs = int(self.price_forecast_config.get('epochs', 30))
                    lr = float(self.price_forecast_config.get('lr', 1e-3))
                    price_series = np.abs(self.charge_prices[0, :])
                    self.price_forecast = generate_price_forecast(
                        price_series,
                        horizon=horizon,
                        lookback=lookback,
                        model=model_name,
                    ...
                    self.price_forecast_horizon = horizon
                    self._price_forecast_cached = True
            except Exception:
                self.price_forecast = None
                self.price_forecast_horizon = None
                self._price_forecast_cached = False
    ...
    def step(self, actions, visualize=False):
        ...
        for i, ev in enumerate(self.EVs_profiles[counter:]):
            if ev.time_of_arrival == self.current_step + 1:
                ...
                target_zone = ev.location
                if self.wireless_enabled:
                    if not self.charging_stations[target_zone].has_free_port():
                        free_zones = [
                            zone.id for zone in self.charging_stations
                            if zone.has_free_port()
                        ]
                        if free_zones:
                            target_zone = int(np.random.choice(free_zones))
                        else:
                            # No wireless zone capacity; skip connecting this EV.
                            self.total_evs_spawned += 1
                            continue

                    ev.location = target_zone

                index = self.charging_stations[target_zone].spawn_ev(ev)
        ...
        self._update_wireless_zone_assignments()
    ...
    def _sample_next_wireless_zone(self, current_zone_id):
        if self.cs <= 1:
            return current_zone_id

        adjacent_only = self.wireless_config.get('adjacent_only', True)
        if adjacent_only:
            candidates = []
            if current_zone_id > 0:
                candidates.append(current_zone_id - 1)
            if current_zone_id < self.cs - 1:
                candidates.append(current_zone_id + 1)
            if not candidates:
                return current_zone_id
            return int(np.random.choice(candidates))

        candidates = [zone_id for zone_id in range(self.cs) if zone_id != current_zone_id]
        return int(np.random.choice(candidates))

    def _update_wireless_zone_assignments(self):
        if not self.wireless_enabled:
            return
        if not self.wireless_config.get('movement_enabled', False):
            return

        move_probability = self.wireless_config.get('move_probability', 0.0)
        if move_probability <= 0:
            return

        for current_zone in self.charging_stations:
            connected_evs = [ev for ev in current_zone.evs_connected if ev is not None]
            for ev in connected_evs:
                if np.random.rand() >= move_probability:
                    continue

                next_zone_id = self._sample_next_wireless_zone(current_zone.id)
                if next_zone_id == current_zone.id:
                    continue

                next_zone = self.charging_stations[next_zone_id]
                if not next_zone.has_free_port():
                    continue

                current_zone.remove_ev(ev)
                ev.location = next_zone_id
                port_index = next_zone.spawn_ev(ev)

                if not self.lightweight_plots:
                    self.port_arrival[f'{next_zone_id}.{port_index}'].append(
                        (self.current_step + 1, ev.time_of_departure + 1)
                    )
    ...
    def _apply_dynamic_discharge_prices(self):
        dynamic_cfg = self.config.get('dynamic_price', {})
        if not dynamic_cfg.get('enabled', False):
            return

        # Cache base discharge prices and reference factors once.
        if not hasattr(self, "_base_discharge_prices"):
            self._base_discharge_prices = self.discharge_prices.copy()
        else:
            self.discharge_prices = self._base_discharge_prices.copy()

        if not hasattr(self, "_dynamic_discharge_factors"):
            reference = dynamic_cfg.get('reference', 'heuristic')
            threshold_kw = float(dynamic_cfg.get('threshold_kw', 40))
            high_factor = float(dynamic_cfg.get('high_factor', 1.1))
            low_factor = float(dynamic_cfg.get('low_factor', 0.9))

            if reference == 'heuristic':
                ref_power = self._compute_heuristic_power_reference()
            else:
                ref_power = self.current_power_usage

            factors = np.where(ref_power > threshold_kw, high_factor, low_factor)
            self._dynamic_discharge_factors = factors
            self._dynamic_discharge_threshold = threshold_kw
            self._dynamic_discharge_factors_config = (high_factor, low_factor)

        self.discharge_prices = self.discharge_prices * self._dynamic_discharge_factors

    def _compute_heuristic_power_reference(self):
        # Build a temporary environment without dynamic pricing to avoid recursion.
        temp_env = EV2Gym(
            config_file=self.config_file,
            save_replay=False,
            save_plots=False,
            generate_rnd_game=self.generate_rnd_game,
            seed=self.seed,
            skip_dynamic_price=True,
        )
        # Disable dynamic pricing and forecasting in the reference env
        temp_env.config['dynamic_price'] = {'enabled': False}
        temp_env.config['price_forecast'] = {'enabled': False}
        temp_env.price_forecast_enabled = False
        temp_env.wireless_config = temp_env.config.get('wireless_charging', {})
        temp_env.wireless_enabled = temp_env.wireless_config.get('enabled', False)
        _ = temp_env.reset()
        ...
        for _ in range(temp_env.simulation_length):
            ...
            _, _, done, truncated, _ = temp_env.step(actions)
            if done or truncated:
                break

        return temp_env.current_power_usage.copy()
