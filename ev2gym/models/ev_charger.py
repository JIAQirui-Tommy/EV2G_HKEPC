...
'''
    ...
    def __init__(self,
        ...
        self.is_wireless_zone = is_wireless_zone
    ...
    def step(self, actions, charge_price, discharge_price):
        ...
        for i, action in enumerate(normalized_actions):
            ...
            if self.current_total_amps - 0.0001 > self.max_charge_current:
                if self.is_wireless_zone:
                    # Wireless zones can aggregate many vehicles; allow a small numerical overshoot.
                    self.current_total_amps = min(self.current_total_amps, self.max_charge_current)
                else:
                    raise Exception(
    ...
    def has_free_port(self):
        return self.n_evs_connected < self.n_ports

    def remove_ev(self, ev):
        for index, connected_ev in enumerate(self.evs_connected):
            if connected_ev is ev:
                self.evs_connected[index] = None
                self.n_evs_connected -= 1
                return index
        return None
