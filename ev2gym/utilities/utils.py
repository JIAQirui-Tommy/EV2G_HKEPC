...
def spawn_single_EV(env,
    ...
    evening_v2g_cfg = env.config.get("workplace_evening_v2g", {})
    if evening_v2g_cfg.get("enabled", False) and scenario == "workplace":
        start_hour = evening_v2g_cfg.get("start_hour", 18)
        end_hour = evening_v2g_cfg.get("end_hour", 23)
        if start_hour <= hour <= end_hour:
            soc_min = float(evening_v2g_cfg.get("arrival_soc_min", 0.5))
            soc_max = float(evening_v2g_cfg.get("arrival_soc_max", 0.7))
            soc_min = max(0.0, min(1.0, soc_min))
            soc_max = max(soc_min, min(1.0, soc_max))
            target_arrival_capacity = np.random.uniform(soc_min, soc_max) * battery_capacity
            initial_battery_capacity = max(initial_battery_capacity, target_arrival_capacity)
            initial_battery_capacity = min(initial_battery_capacity, battery_capacity)
...
def EV_spawner(env) -> List[EV]:
    ...
    spawn_window_cfg = env.config.get("ev_spawn_window", {})
    weekday_start_hour = spawn_window_cfg.get("weekday_start_hour", 6)
    weekday_end_hour = spawn_window_cfg.get("weekday_end_hour", 18)
    weekend_enabled = spawn_window_cfg.get("weekend_enabled", False)
    weekend_start_hour = spawn_window_cfg.get("weekend_start_hour", 0)
    weekend_end_hour = spawn_window_cfg.get("weekend_end_hour", 23)
    ...
    for t in range(2, env.simulation_length-min_time_of_stay_steps-1):
        ...
        if day < 5:
            if scenario == "workplace" and (hour < weekday_start_hour or hour > weekday_end_hour):
            ...
            if scenario == "workplace":
                if not weekend_enabled or hour < weekend_start_hour or hour > weekend_end_hour:
                    time = time + datetime.timedelta(minutes=env.timescale)
                    continue
            else:
