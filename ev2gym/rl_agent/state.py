...
def V2G_profit_max_forecast(env, *args):
    """
    Same as V2G_profit_max but uses learned price forecasts when available.
    """
    state = [
        (env.current_step),
    ]
    ...
    horizon = getattr(env, 'price_forecast_horizon', 20) or 20
    if hasattr(env, 'price_forecast') and env.price_forecast is not None:
        idx = min(env.current_step, env.price_forecast.shape[0] - 1)
        forecast = env.price_forecast[idx, :horizon]
        if len(forecast) < horizon:
            forecast = np.append(forecast, np.zeros(horizon - len(forecast)))
        state.append(forecast)
    else:
        charge_prices = abs(env.charge_prices[0, env.current_step:
    ...
    state = np.array(np.hstack(state), dtype=float)
    state = np.nan_to_num(state, nan=0.0, posinf=0.0, neginf=0.0)
    return state


def V2G_profit_max_price_load_forecast(env, *args):
    """
    State with both price forecast and transformer load forecast.
    """
    state = [
        (env.current_step),
    ]
    ...
    for tr in env.transformers:
        loads, pv = tr.get_load_pv_forecast(step=env.current_step, horizon=horizon)
